...
@section('content')
                ...
                <div class="bg-white rounded-lg shadow-sm p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900">{{ $matches->where('status', 'pending')->count() }}
                            </p>
                        </div>
                    </div>
                </div>
                <div class="bg-white rounded-lg shadow-sm p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900">{{ $matches->where('status', 'approved')->count() }}
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900">
                                {{ $matches->where('status', 'scheduled')->count() }}</p>
                        </div>
                    </div>
                </div>
                <div class="bg-white rounded-lg shadow-sm p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900">
                                {{ $matches->where('status', 'completed')->count() }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Filters -->
            <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                <form method="GET" action="{{ route('recipient.matches.index') }}"
                                    ...
                                    @foreach ($matches as $match)
                                        <tr>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm font-medium text-gray-900">{{ $match->foodListing->food_name }}
                                                </div>
                                                <div class="text-sm text-gray-500">
                                                    {{ Str::limit($match->foodListing->description, 50) }}</div>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900">
                                                    {{ $match->foodListing->restaurantProfile->restaurant_name }}</div>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <div class="flex flex-col space-y-2">
                                                    ...
                                                    @if ($match->status == 'scheduled')
                                                        <form method="POST"
                                                            action="{{ route('recipient.matches.confirmPickup', $match->id) }}">
                                                            @csrf
                                                    ...
                                                    @if (in_array($match->status, ['pending', 'approved']))
                                                        <form method="POST"
                                                            action="{{ route('recipient.matches.cancel', $match->id) }}">
                                                            @csrf
                                                            @method('DELETE')
